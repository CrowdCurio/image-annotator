...
ImageAnnotator.prototype.initialize = function (config) {
  ...
  if(!window.assessment){
      ...
      this.client.create('annotation', {//to mark that the worker has watched the tutorial video
        ...
        console.log('new watched video annotaiton id: '+annotation.id)
    });
  ...
  function preventDefaultForScrollKeys(e) {
    ...
    if(event.originalEvent.deltaY < 0){
        if ($('#zoom').val() != '2' && $('#zoom').val() != 2.0) {if(!pauseZoom){ms.zoom_in(event);pauseZoom=true;window.setTimeout(function(){ pauseZoom=false; }, 150);}  }
    }
    else {
        if ($('#zoom').val() != '0.0625') { if(!pauseZoom){ms.zoom_out(event);pauseZoom=true; window.setTimeout(function(){ pauseZoom=false; }, 150);}  }
    }
});
};

ImageAnnotator.prototype.postInitialize = function (config) {
  if(window.assessment == 'false' || window.assessment == false){
    const that = this;
    ...
    that.onNextTask = function () {
        ...
        if(that.timer.getTime()<window.seconds_before_submit*1000){
            swal({
              ...
              text: 'Please take some time to examine the image. \nThank you!', //
              type: 'warning',
    ...
    $('#exit-button').click((e) => {
        swal({
             confirmButtonColor: '#39ce63',
             type: 'warning',
             title: 'Are you sure you want to quit now?',
             html: "<br>Please note that if you quit now you will be compensated for only the <strong>complete sets</strong> you counted, not including the current set.",
             showCancelButton: true,
             cancelButtonColor: '#d33',
             confirmButtonText: 'Yes',
             cancelButtonText: 'No',
        ...
        .then((result) => {
              if(result.value){
                ...
                this.client.listAll('data', {                                   //unassign the assigned data
                                  ...
                                  unassignDataOne.push(images[i].id);
                                  }
                                  for(var i=0;i<unassignDataOne.length;i++){
                                  dataid = unassignDataOne[i].toString();
                                  console.log('freeing one: '+dataid)
                                  this.client.update('data', {
                                                     id: dataid,
                                                     assignedtoone: '0',
                                                     }, (result) => {
                                                     console.log(unassignDataOne[i].toString()+' assignedtoone is freed.')
                                                     });
                                  }

                                  this.client.listAll('data', {
                                                      experiment: window.experiment,
                                                      testassignedtotwo: window.user,
                                                      }, (images) => {
                                                      for(var i=0; i<images.length; i++){
                                                      console.log('looping through images: ')
                                                      unassignDataTwo.push(images[i].id);
                                                      }
                                                      for(var i=0;i<unassignDataTwo.length;i++){
                                                      dataid = unassignDataTwo[i].toString();
                                                      console.log('freeing two: '+dataid)
                                                      this.client.update('data', {
                                                                         id: dataid,
                                                                         assignedtotwo: '0',
                                                                         }, (result) => {
                                                                         console.log(unassignDataTwo[i].toString()+' assignedtotwo is freed.')
                                                                         });
                                                      }

                                                      this.client.listAll('data', {
                                                                          experiment: window.experiment,
                                                                          testassignedtothree: window.user,
                                                                          }, (images) => {
                                                                          for(var i=0; i<images.length; i++){
                                                                          console.log('looping through images: ')
                                                                          unassignDataThree.push(images[i].id);
                                                                          }
                                                                          for(var i=0;i<unassignDataThree.length;i++){
                                                                          dataid = unassignDataThree[i].toString();
                                                                          console.log('freeing three: '+dataid)
                                                                          this.client.update('data', {
                                                                                             id: dataid,
                                                                                             assignedtothree: '0',
                                                                                             }, (result) => {
                                                                                             console.log(unassignDataThree[i].toString()+' assignedtothree is freed.')
                                                                                             });
                                                                          }
                                                                          setTimeout(() => {
                                                                                     incrementExperimentWorkflowIndex(csrftoken, window.user, window.experiment, window.hitid);
    ...
    $('.locked-overlay').click(() => {
        ...
        if(passTest=='True'){
        //incrementExperimentWorkflowIndex fixit
             swal({
                  confirmButtonColor: '#39ce63',
                  title: 'Congrats, you have passed the tests already.',
                  text: "You will be redirected to the paid tasks.",
                  type: 'success',
                  confirmButtonText: 'Got it',
                  allowEscapeKey: false,
                  allowEnterKey: false,
            ...
            const taskSwitched = window.localStorage.getItem(`crowdcurio-tau-${window.experiment}-task-switch`);//fixit begin
            if (!config.default_queue && !taskSwitched) {
                this.currentQueue = 'required';
            } else if (taskSwitched) {
                this.currentQueue = taskSwitched;
            ...
            swal({
                        ...
                        html: "<br>To start the actual task, you need to pass one of the two tests with an " +window.dejavu_passing_score.toString()+ " or higher accuracy . <br><br>You have 1 chance for the first and 3 chances for the seconds test. <br><br>If you fail the first test, the solution will be provided; please examine it carefully.",
                            confirmButtonColor: '#39ce63',
                          confirmButtonText: 'Got it',
                          allowEscapeKey: false,
                          allowEnterKey: false,
            ...
            this.client.getNextTask(this.currentQueue, (task) => {
                if(task.id == null){
                    swal({
                        title:'Sorry, all images have already been assigned to workers.',
                        html: "<br>You will be redirected to MTurk's homepage. <br>Please check back again soon!",
                            confirmButtonColor: '#39ce63',
                          confirmButtonText: 'Got it',
                          allowEscapeKey: false,
                          allowEnterKey: false,
                         ...
                         location.replace("https://www.mturk.com")
                      })
                }
                else{
        ...
        }else if(test1Attempts>=1 && test2Attempts>=3){//should be rejected to participate in this HIT
                swal({
                  type: 'warning',
                  title: 'Sorry, you are not qualified to participate in this HIT.',
                  text: 'You will be redirected to MTurk\'s homepage. Thank you.',
                  confirmButtonColor: '#39ce63',
    ...
    const that = this;
    if(window.assessment && test1Attempts>=1 && test2Attempts<3){//should go to second test
                that.completedTasks = parseInt(that.completedTasks) + 1;
    }
    ...
    if(testTasks.length == 0){
        ...
        for(var i=0; i<3;i++){//dynamically load the example images.
            for(j = 0; j<3; j++){
                var elementID = "example-img-"+genres[i]+parseInt(j+1).toString();
                console.log(elementID)
                console.log(window.config.labels[testTasks[that.completedTasks].genre.toLowerCase()][i].examples[j])
                console.log('\n')
                document.getElementById(elementID).innerHTML = "<img src="+config.labels[testTasks[that.completedTasks].genre.toLowerCase()][i].examples[j]+">";
            }
        }
    ...
    that.onNextTask = function (score, bCount,bSol,fCount,fSol,frCount,frSol, sequence, passedTest) {
      ...
      incrementExperimentWorkflowIndex(csrftoken, window.user, window.experiment, window.hitid);
            ...
            if(sequence==1){
                ...
                swal({
                  type: 'warning',
                  title: 'Sorry, you did not pass this test.',
                  html: 'You scored '+ score.toFixed(2) +'% by marking:<br>'+bCount+' out of '+bSol+' buds<br>'+fCount+' out of '+fSol+' flowers<br>'+frCount+' out of '+frSol+' fruits<br><br>Please carefully examine the solution which will be provided to you. <br><br>For the second test you will have 3 chances.',
                  confirmButtonColor: '#39ce63',
                  //type: 'warning',
                  showCancelButton: false,
                  cancelButtonColor: '#d33',
                ...
                test2Attempts= parseInt(test2Attempts)+1;
                if(test2Attempts>=3){
                    swal({
                      type: 'warning',
                      html: 'Sorry, have used up all your chances. <br><br>You will be redirected to MTurk\'s homepage. <br><br>Thank you for your participation!',
                      confirmButtonColor: '#39ce63',
                      showCancelButton: false,
                      cancelButtonColor: '#d33',
                      confirmButtonText: 'Got it',
                      ...
                      allowOutsideClick:  false,
                        ...
                        this.modals.experiment_complete_modal.modal('open');
                    });
        ...
        var steps = [
          {
            ...
            html: 'You scored '+ score.toFixed(2) +'% by marking:<br>'+bCount+' out of '+bSol+' buds<br>'+fCount+' out of '+fSol+' flowers<br>'+frCount+' out of '+frSol+' fruits',
            confirmButtonColor: '#39ce63',
            confirmButtonText: 'Got it',
            allowOutsideClick:    false,
            allowEscapeKey: false,
        ...
        swal.queue(steps).then(function() {
          if(score != 100){
            //alert("The solution will be opened in a new tab.");
            swal({
              title: 'The solution will be opened in a new tab.',
              text: "",
              confirmButtonColor: '#39ce63',
              //type: 'warning',
              showCancelButton: false,
    ...
    /* attach the handler for hte submit / next-task button */
    $('#next-button').click((e) => {
      var bCount = parseInt(document.getElementById("bud-count").innerHTML);
      var fCount = parseInt(document.getElementById("flower-count").innerHTML);
      var frCount = parseInt(document.getElementById("fruit-count").innerHTML);

      var counts = {bCount,fCount, frCount}

      var bSol =  parseInt(ad[that.completedTasks]["abc"]["a"]) + 10;
      var fSol  = parseInt(ad[that.completedTasks]["abc"]["b"]) - 200;
      var frSol = parseInt(ad[that.completedTasks]["abc"]["c"]) + 3000;
      var bWro = parseInt(Math.abs(bCount-bSol));
      var fWro = parseInt(Math.abs(fCount-fSol));
      var frWro = parseInt(Math.abs(frCount-frSol));
      var bCor = Math.min(bSol,bCount)
      var fCor = Math.min(fSol,fCount)
      var frCor = Math.min(frSol,frCount)
      //var score = 100 - 100*(  Math.abs((bSol+fSol+frSol)-(bSol+fSol+frSol))/(bSol+fSol+frSol+bSol+fSol+frSol)  ) only aggregate score
      ...
      if(score < window.test_passing_score){
        this.client.create('annotation', {
            ...
            console.log('new annotaiton id: '+annotation.id)
        });
      }else{
        this.client.create('annotation', {
            hitid: window.hitid,
...
ImageAnnotator.prototype.attachHandlers = function () {
  ...
  /* zoom handlers */
  $('#zoom_in').click((event) => { if ($('#zoom').val() != '2' && $('#zoom').val() != 2.0) { ms.zoom_in(event); } });
  $('#zoom_out').click((event) => { if ($('#zoom').val() != '0.0625') { ms.zoom_out(event); } });
...
ImageAnnotator.prototype.render = function (config) {
    ...
    if ('active' in config.collaboration) {
      if (config.collaboration.active) {
        ...
        if (config.collaboration.style === 'self-image') {
          modalsTemplate = '<div id="loading_modal" class="modal" style="top: auto; width: 310px !important;"><div class="modal-content modal-trigger" href="#loading_modal" style="height:100px;"><h5>Loading Task Interface</h5><div class="progress"><div class="indeterminate"></div></div></div></div><div id="experiment_complete_modal" class="modal" style="top: auto; width: 310px !important;"><div class="modal-content modal-trigger" href="#experiment_complete_modal" style="height: 110px;"><center><h5>Leaving Task</h5></center><div class="progress"><div class="indeterminate"></div></div></div></div><div id="fetching_task_modal" class="modal" style="top: auto; width: 310px;"><div class="modal-content modal-trigger" href="#fetching_task_modal" style="height: 110px;text-align: center;"><h5 id="fetching-task-modal-text">Getting Next Task</h5><div class="progress"><div class="indeterminate"></div></div></div></div><div id="finished_modal" class="modal" style="top: auto; width: 310px;"><div class="modal-content modal-trigger" href="#finished_modal" style="height: 110px;text-align: center;"><font size="6" id="fetching-task-modal-text">This task has been completed. Thank you! </font><hr/><div><img src=\"https://media.giphy.com/media/j5QcmXoFWl4Q0/giphy.gif\"/></div><hr/><div class=\'row\' style=\'margin-bottom: 0px;\'><h5>You are being redirected to the next step. </h5></div></div></div><div id="practice_finished_modal" class="modal" style="top: auto; width: 310px;"><div class="modal-content modal-trigger" href="#practice_finished_modal" style="height: 110px;text-align: center;"><font size="6" id="fetching-task-modal-text">This task has been completed. Thank you! </font><hr/><div>You\'ve completed all available practice tasks.</div><hr/><div class=\'row\' style=\'margin-bottom: 0px;\'><p>Exiting the Practice Room</p></div></div></div><div id="practice_information_modal" class="modal" style="top: auto; width: 510px;height: 565px;"><div class="modal-content" href="#practice_information_modal" style="height: 110px;text-align: center;"><h5 id="fetching-task-modal-text">What is the Practice Room?</h5><hr/><div class="practice-information-body">The Practice Room is a task mode that lets you practice tasks without being penalized. The mode facilitates three objectives: <center><img src="https://www.burkert.com/var/buerkert/storage/images/media/images/speech-bubbles/3211190-1-eng-INT/speech-bubbles.jpg" style="width: 300px;"></center><ul><li><b>Practice tasks tell you what you did correctly.</b> You can try the task and compare your answers to what\'s correct.</li><hr style="width: 100px;"/><li><b>Practice tasks don\'t count against you:</b> However, practice tasks don\'t count toward the tasks required to complete the HIT.</li><hr style="width: 100px;"/><li><b>You can practice at your own leisure:</b> You can switch to the Practice Room at any point during the task. If you switch modes in the middle of a task, your annotations won\'t be erased.</li></ul></div><hr/><div class=\'row\' style=\'margin-bottom: 0px;\'><p><a id="practice-information-button" class="modal-action modal-close btn">Got it!</a></p></div></div></div><div id="survey_efficacy_modal" class="modal" style="top: auto; width: 710px;height: 365px;"><div class="modal-content" href="#survey_efficacy_modal" style="height: 110px;text-align: center;"><h5 id="fetching-task-modal-text">Rate Yourself!</h5><hr/><div class="self-efficacy-survey-body"><center><p style="width: 500px;">On a scale of 1 to 10, how confident are you that you can perfectly identify all the Greek Taus in the next image?</p></center><input id="self-efficacy-slider" type="range" min="1" max="10" step="1" value="5"/><center><p style="font-size: 0.7em; margin: 0;">Note: Your answer will not affect your payment.</p></center></div><hr/><div class=\'row\' style=\'margin-bottom: 0px;\'><p><a id="efficacy-submit-button" class="modal-action modal-close btn">Submit</a></p></div></div></div>';
        } else { // 2. passive
  ...
  if (config.mode.toLowerCase() === 'transcription') {
    ...
    let examples = '';
    for (var i = 0; i < 3; i++) {
      let example = `<div id="example-container-${genres[i]}" class="example-container" style="display: none;">`;
      for (let j = 0; j < 3; j++) {
        //example += `<div class="example-img"><img src="${config.labels[task.genre][i].examples[j]}"/></div>`;
        example += `<div class='example-img' id="example-img-${genres[i]}${j+1}"><img src=""></div>`;
      }
      example += '</div>';
      examples += example;
    }
  ...
  if (surface.mode == 'transcription') {
    ...
    if (((new Date().getTime()) - surface.timeOfLastDrag) > 100 && !surface.hoveringOnMarker) {
      ...
      if ($('.task-option-toggle.success').length == 0) {
        //alert('In order to start counting, please click on one of the three labels on the right of the page (e.g. Flower).');
        swal({
          title:'Please choose an organ first.',
          confirmButtonColor: '#39ce63',
          text:'In order to start counting, please click on one of the three labels on the right of the page (e.g. flower).',
          type:'warning',
          allowEscapeKey: false,
          allowEnterKey: false,
          allowOutsideClick:    false,
      ...
      //relocating the newly created marker
      var scaler = 0;
      if( zoom === 0.0625){
          scaler = 2.3;
      } else if (zoom === 0.125) {
...
//  }
//  ele.animate({
//    top: `${zoom * (parseInt(marker.y) - (31 * scaler))}px`, left: `${zoom * (parseInt(marker.x) - (31 * scaler))}px`, height: '60px', width: '60px', 'border-radius': '100px',
//  }, 50);
//
...
ImageAnnotator.prototype.zoom = function (current_factor, factor) {
  ...
  // console.log("Updating Zoom Factor to: "+factor);
    console.log("new Wdith: "+$('#fragment img').width());
   //Scale fragment container position
    var fc_top = $("#fragment_container").position().top;
    var fc_left = $("#fragment_container").position().left;
    var wr = $('.location').width() / $('#thumb').width() ;
    var hr = $('.location').height() / $('#thumb').height() ;
    //zoom in/out along with the center of the screen instead of the top left corner
    fc_top = (fc_top/current_factor)*factor + (hr *  $('#fragment img').height()/factor * current_factor / 2 ) - (hr *  $('#fragment img').height() / 2 );
    fc_left = (fc_left/current_factor)*factor + (wr *  $('#fragment img').width()/factor * current_factor / 2 ) - (wr *  $('#fragment img').width() / 2 );
...
ImageAnnotator.prototype.zoom_in = function (event) {
  ...
  factor = parseFloat(current_factor) + parseFloat($('#zoom_step').val());
  if(Math.abs(parseFloat(current_factor)-0.25) <0.1){
      factor = 0.5;
  }else if(Math.abs(parseFloat(current_factor)-0.125)<0.01) {
     factor = 0.25;
  }else if(Math.abs(parseFloat(current_factor)-0.0625)<0.01){
     factor = 0.125
  }else{
     factor = parseFloat(current_factor) + parseFloat($('#zoom_step').val());
  }
  ...
  event.preventDefault();
  var markers = document.getElementsByClassName("marker")//keep the markers sizes
  zoom = $('#zoom').val();
  if( zoom == 0.0625){
     scaler = 2.3;
  } else if (zoom == 0.125) {
...
ImageAnnotator.prototype.zoom_out = function (event) {
  ...
  const current_factor = $('#zoom').val();
  factor = 0;
  if(Math.abs(parseFloat(current_factor)-0.5) <0.1){
          factor = 0.25;
  }else if(Math.abs(parseFloat(current_factor)-0.25)<0.01) {
          factor = 0.125;
  }else if(Math.abs(parseFloat(current_factor)-0.125)<0.01){
          factor = 0.0625
  }else{
  ...
  if( zoom == 0.0625){
    ...
    console.log("zoom out scaler is: "+scaler)
  for (var i=0; i<markers.length; i++){
    markers[i].style.height = '25px';
    markers[i].style.width = '25px';
    if(zoom <= 0.5){
...
ImageAnnotator.prototype.toggleFullscreen = function (e) {
  ...
  if (main_interface.hasClass('fullscreen')) {
    ...
    document.getElementById("fullscreen-btn-icon").className = "fa fa-arrows-alt";
    ...
    document.getElementById("fullscreen-btn-icon").className = "fa fa-compress-arrows-alt";
