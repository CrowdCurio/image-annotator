...
ImageAnnotator.prototype.postInitialize = function (config) {
  if(!window.assessment){
    ...
    if ($('#main-interface').hasClass('fullscreen')){//fixit
    ...
    that.onNextTask = function () {
      ...
      var score = ((100 - Math.abs(bSol - bCount)/bSol*100) + (100 - Math.abs(fSol - fCount)/fSol*100) + (100 - Math.abs(frSol - frCount)/frSol*100))/3;
      ...
      if(!( Math.abs(bSol - bCount) <= 0.2 * bSol) || !( Math.abs(fSol - fCount) <= 0.2 * fCount) || !( Math.abs(frSol - frCount) <= 0.2 * frSol)){
        ...
        var solUrl = testTasks[that.completedTasks].solurl;//jarvis delete this
        var steps = [
          {
        ...
        //Jarvis
        //1. create an image with the markers which adds a layer to the current image
        //2. create a new website to include the image in solUrl and the image above
        //3. change the window.open(...) link to the page created in step 2 
        ...
        swal.queue(steps).then(function() {
          if(score != 100){
            ...
            var solArr = [];
            solArr.push({x: 820, y:120, label: 'flower' });
            solArr.push({x: 502, y:1020, label: 'fruit' });
            solArr.push({x: 1320, y:1020, label: 'fruit' });
            solArr.push({x: 5000, y:1900, label: 'flower' });
            solArr.push({x: 799, y:800, label: 'flower' });
            solArr.push({x: 102, y:1800, label: 'fruit' });
            solArr.push({x: 1000, y:700, label: 'bud' });
            solArr.push({x: 420, y:1020, label: 'fruit' });
            solArr.push({x: 1220, y:1728, label: 'fruit' });
