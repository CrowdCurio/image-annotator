  ...
  if(!window.assessment){
    ...
    that.onNextTask = function () {
      ...
      if(!( Math.abs(bSol - bCount) <= 0.2 * bSol) || !( Math.abs(fSol - fCount) <= 0.2 * fCount) || !( Math.abs(frSol - frCount) <= 0.2 * frSol)){
        ...
        var solUrl = testTasks[that.completedTasks].solurl;
        var steps = [
          {
...
ImageAnnotator.prototype.attachHandlers = function () {
  ...
  // $('.task-option-toggle').click((e) => {
    ...
    if (!$(`#${e.currentTarget.id}`).hasClass('success')) {
      ...
      $(`#${e.currentTarget.id}`).removeClass('success'); //debug jarvis
      $('.example-container').hide();
    }
...
ImageAnnotator.prototype.render = function (config) {
  ...
  const nextWindowTemplate = '<div id="submission_toggles" class="submission side_buttons" style="float: left; width: 250px; min-height: 100px;"> <div id="controls">  <div id="submission_toggles_inner"> <div class="toggle-text"> Next Task <hr/> </div> <button id="next-button" class="waves-light btn submit"> <i class="fa fa-arrow-right" aria-hidden="true"></i> </button> </div> <div class="view-divider" style="border-top: 1px solid black; border-bottom: initial;height: 4px;width: 100%;"></div> </div> </div>';
...
//jarvis
  ...
  let rightActionPanel = '<div id="right-action-panel"></div><div id="bottom-controls"> <div id="bottom-control-keypad" style="float: left;display: inline-block;height: 100%;"> {LABEL_SPACE} </div> </div>';
  ...
  if (config.mode.toLowerCase() === 'transcription') {
    ...
    for (var i = 0; i < config.labels.length; i++) {
      labels += `<label id="${config.labels[i].name.toLowerCase()}-btn" class="task-option-toggle toggle-btn" lval="marker-n${i + 1}"><input type="radio" name="${i + 1}" value="${config.labels[i].name}"/>${config.labels[i].name}<div id="${config.labels[i].name.toLowerCase()}-count" class="task-option-count" marker="n${i + 1}" >?</div></label><i id="${config.labels[i].name.toLowerCase()}-estimate" class="white-text estimate-button fa fa-pencil-square-o fa-1" style="display: ${estimate_display};color:black;float: right;padding-top: 12px;cursor: pointer;"></i><br>`;
    }
    labels += '</div>';
...
ImageAnnotator.prototype.toggleFullscreen = function (e) {
  ...
  rightActionPanel = $('#right-action-panel');//jarvis not constant since we assign a panel to it
