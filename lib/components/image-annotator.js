...
ImageAnnotator.prototype.postInitialize = function (config) {
  if(!window.assessment){
    ...
    that.onNextTask = function () {
        ...
        if(that.timer.getTime()<60000){
            swal({
              ...
              text: 'Please take at least one minute to examine each image. \nThank you!',
              type: 'warning',
    ...
    that.onNextTask = function () {
      ...
      var bSol = ad[that.completedTasks]["abc"]["a"];
      var fSol = ad[that.completedTasks]["abc"]["b"];
      var frSol = ad[that.completedTasks]["abc"]["c"];
      ...
      var score = Math.abs( bScore + fScore + frScore )/3;
      
      var bDiff = Math.abs(bSol - bCount);
      var fDiff = Math.abs(fSol - fCount);
      var frDiff = Math.abs(frSol - frCount);
      ...
      if(!( bDiff <= 0.2 * bSol || bDiff <= 3) || !( fDiff <= 0.2 * fCount || fDiff <= 3) || !( frDiff <= 0.2 * frSol || frDiff <= 3) || score<50){
          swal({
        ...
        var steps = [
          {
            ...
            html: 'You scored '+ score.toFixed(0) +'%.<br><br>You marked:<br>'+bCount+' out of '+bSol+' buds<br>'+fCount+' out of '+fSol+' flowers<br>'+frCount+' out of '+frSol+' fruits',
            confirmButtonColor: '#39ce63',
            confirmButtonText: 'Got it',
            allowOutsideClick:	false,
            allowEscapeKey: false,
        ...
        swal.queue(steps).then(function() {
          if(score != 100){
            ...
            .then((result) => {
                ...
                orgimg.sArr = ad[that.completedTasks]["coordinates"];
...
ImageAnnotator.prototype.addTool = function (event) {
  if (surface.mode == 'transcription') {
    ...
    if (((new Date().getTime()) - surface.timeOfLastDrag) > 100 && !surface.hoveringOnMarker) {
      ...
      /*  append the draggable element into html */
      var newMarker;
      console.log("debug new marker: mx: "+mx+", my: "+my);
      mx = (mx + 10 - (10 * scaler));
      my = (my + 10 - (10 * scaler));
      if (typeof recording !== 'undefined') {
        $('.overlay.selected').hide();
...
ImageAnnotator.prototype.hoverToolOn = function (e) {
  console.log("debug hover tool on")
...
ImageAnnotator.prototype.hoverToolOff = function (e) {
  console.log("debug hover tool off")
  if (surface.movingMarker) {
    return;
  }
  this.hoveringOnMarker = false;
...
//
//  ele.animate({
//    top: `${parseFloat(zoom * (parseFloat(marker.y) - (10 * scaler)))}px`, left: `${parseFloat(zoom * (parseFloat(marker.x) - (10 * scaler)))}px`, height: '20px', width: '20px',
//  }, 50);
//
